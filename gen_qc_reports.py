...
def dadab(rec, metric):
    if metric == "DDDs":
        ...
        if v is None:
            return None
        t = ddds_targets.get(norm_key(rec["科室"]))
...
def draw_radar(dept_rec, group_name, metrics, out_path):
    ...
    for angle, radius, m in zip(angles[:-1], radii, filt):
        ...
        ax.text(angle, 1.25, m, ha='center', va='center', fontsize=7, fontproperties='SimHei',
    ...
    ax.set_yticklabels(['D', 'C', 'B', 'A'], fontsize=8, fontweight='bold', fontproperties='SimHei')
    ...
    ax.set_title(f"{dept_rec['科室']} — {group_name}", fontsize=11, fontweight='bold',
                 pad=20, color='#1F3A5F', fontproperties='SimHei')
